...
static int g_object = 1; // Sky camera: 0, cube 1: 1, cbe 2: 2
...
static Matrix4 g_objectRbt[2] = {Matrix4::makeTranslation(Cvec3(-.75,0,0)), Matrix4::makeTranslation(Cvec3(.75,0,0))};  // 2 objects defined
static Cvec3f g_objectColors[2] = {Cvec3f(1, 0, 0), Cvec3f(0, 0, 1)}; //Red, Blue

static Matrix4 g_viewRbt = g_skyRbt;
static Matrix4 g_objRbt = g_objectRbt[0];
...
static void drawStuff() {
  ...
  const Matrix4 eyeRbt = g_viewRbt;
...
static void motion(const int x, const int y) {
  ...
  if (!(g_object == 0 && g_view != 0)) {
    if (g_mouseLClickButton && !g_mouseRClickButton) { // left button down?
      ...
      // Take M to O w.r.t Origin
      if (g_object == 0 && g_view == 0 && g_skyFrame == 0) {
        g_objRbt = m * transFact(g_objRbt) * linFact(g_viewRbt);
      }
      // Take M to O w.r.t mixedFrame
      else {
        Matrix4 mixedFrame = transFact(g_objRbt) * linFact(g_viewRbt);
...
static void reset()
{
    ...
    g_objectRbt[0] = Matrix4::makeTranslation(Cvec3(-.75,0,0));
    g_objectRbt[1] = Matrix4::makeTranslation(Cvec3(.75,0,0));
    g_viewRbt = g_skyRbt;
