...
inline Quat pow(const Quat& q, const double exponent) {
  double length = sqrt(q[1]*q[1] + q[2]*q[2] + q[3]*q[3]);
  double theta = atan2(length, q[0]);
  // Avoid divide by zero.
  Quat q1 =  Quat(cos(exponent*theta), sin(exponent * theta) * q[1], sin(exponent * theta) * q[2], sin(exponent * theta) * q[3]);
  if (length != 0) {
    q1 /=length;
    q1[0] *= length;
  }
