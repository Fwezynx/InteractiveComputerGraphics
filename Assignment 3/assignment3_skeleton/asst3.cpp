...
static void motion(const int x, const int y) {
  ...
  if (!(g_object == 0 && g_view != 0)) {
    if (g_mouseLClickButton && !g_mouseRClickButton) { // left button down?
      if (!(g_object == g_view && g_object != 0)) {
        Cvec2 arcballCoords;
        if (g_object == 0) {
          arcballCoords = getScreenSpaceCoord((inv(g_viewRbt) * RigTForm(Cvec3(0, 0, 0))).getTranslation(), makeProjectionMatrix(), g_frustNear, g_frustFovY, g_windowWidth, g_windowHeight);
        }
        else if (g_object == 1) {
          arcballCoords = getScreenSpaceCoord((inv(g_viewRbt) * g_objRbt).getTranslation(), makeProjectionMatrix(), g_frustNear, g_frustFovY, g_windowWidth, g_windowHeight);
        }
        else {
          arcballCoords = getScreenSpaceCoord((inv(g_viewRbt) * g_objRbt).getTranslation(), makeProjectionMatrix(), g_frustNear, g_frustFovY, g_windowWidth, g_windowHeight);
        }
      ...
      RigTForm mixedFrame = transFact(g_objRbt) * linFact(g_viewRbt);
