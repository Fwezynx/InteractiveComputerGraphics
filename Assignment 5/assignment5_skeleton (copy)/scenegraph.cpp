...
public:
  ...
  const RigTForm getAccumulatedRbt(int offsetFromStackTop = 0) {
    // return the RBT located at offsetFromStackTop items from the top of the stack.
    return rbtStack_.at(rbtStack_.size() - 1 - offsetFromStackTop);
  }

  virtual bool visit(SgTransformNode& node) {
    // If the stack is empty, push the node in RBT form.  Otherwise, modify the node first.
    if (rbtStack_.empty()) {
