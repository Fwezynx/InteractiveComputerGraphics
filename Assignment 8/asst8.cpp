...
static std::vector<Cvec3> g_tipPos, g_tipVelocity;
...
static shared_ptr<SimpleGeometryPN> g_meshGeometry, g_bunnyGeometry;
...
static shared_ptr<SgRbtNode> g_skyNode, g_groundNode, g_robot1Node, g_robot2Node, g_viewNode, g_light1Node, g_light2Node, g_meshNode, g_bunnyNode;
...
static void updateShellGeometry() {
  ...
  Cvec3 bunnyRbt = getPathAccumRbt(g_world, g_bunnyNode).getTranslation();
  for (int i = 0; i < g_numShells; i++) {
    vertices.clear();
    for (int j = 0; j < g_bunnyMesh.getNumFaces(); j++) {
      for (int k = 0; k < g_bunnyMesh.getFace(i).getNumVertices(); k++) {
        Cvec2 texture;
        if (k == 0) {
          texture = Cvec2(0, 0);
        }
        ...
        if (i == 0) {
            ...
            // position_i = p + sum(n + d*(i-1))
            position = p + (n * i) + (d * i * (i - 1)/2);
        }
        vertices.push_back(VertexPNX(position, n + d * i, texture));
      }
    }
    g_bunnyShellGeometries[i]->upload(&vertices[0], vertices.size());
  }
...
static void initBunnyMeshes() {
  ...
  g_bunnyGeometry->upload(g_bunnyMesh, true);
  // Allocate an array of SimpleGeometryPNX for shells.
...
static void subdivideMeshCatmullClark(Mesh& mesh) {
  ...
  for (int i = 0; i < mesh.getNumVertices(); i++) {
    ...
    // (n-2)/n * v + 1/(n*n) * sum(v) + 1/(n*n) * sum(vf)
    Mesh::VertexIterator iterator(vertex.getIterator());
    Mesh::VertexIterator it(iterator);
    double n = 0;
    Cvec3 sumVfV = Cvec3();
    do {
    sumVfV += iterator.getVertex().getPosition() + mesh.getNewFaceVertex(iterator.getFace());
    n++;
    }
    while (++iterator != it);
    mesh.setNewVertexVertex(vertex, vertex.getPosition() * (n-2)/n + sumVfV/(n*n));
  }
  mesh.subdivide();
}
...
static void initScene() {
  ...
  g_meshNode.reset(new SgRbtNode(RigTForm(Cvec3(0, 2.5, 0))));
  g_meshNode->addChild(shared_ptr<MyShapeNode>(new MyShapeNode(g_meshGeometry, g_meshMat, Cvec3())));

  // createa single transform node for both the bunny and the bunny shells
  g_bunnyNode.reset(new SgRbtNode());

  // add bunny as a shape node
  g_bunnyNode->addChild(shared_ptr<MyShapeNode>(new MyShapeNode(g_bunnyGeometry, g_bunnyMat)));

  // add each shell as shape node
  for (int i = 0; i < g_numShells; i++) {
    g_bunnyNode->addChild(shared_ptr<MyShapeNode>(new MyShapeNode(g_bunnyShellGeometries[i], g_bunnyShellMats[i])));
  }
...
static void reset() {
  ...
  g_msBetweenBubbleFrames = 2000;
  g_smoothShading = false;
  g_subDivisions = 0;
  g_meshGeometry->upload(g_mesh, g_smoothShading);
  g_furHeight = 0.21;
  g_hairyness = 0.7;
  Cvec3 g_gravity(0, -0.5, 0);
  g_timeStep = 0.2;
  g_numStepsPerFrame = 10;
  g_damping = 0.96;
  g_stiffness = 4;
  g_simulationsPerSecond = 60;
...
static void hairSimulationCallback(int dontCare) {
  g_shellNeedsUpdate = true;
  Cvec3 bunnyRbt= getPathAccumRbt(g_world, g_bunnyNode).getTranslation();
  // Perform multiple steps before rendering
  for (int i = 0; i < g_numStepsPerFrame; i++) {
    for (int j = 0; j < g_bunnyMesh.getNumVertices(); j++) {
      ...
      // s and p with respect to world frame
      s=bunnyRbt + s;
      p=bunnyRbt + p;
      // force = gravity + spring force
      Cvec3 springF = (s - t) * g_stiffness;
      Cvec3 f = g_gravity + springF;
      // update tip position
      t = t + v * g_timeStep;
      // Constrain t, don't divide by 0
      if (norm2(t - p) != 0) {
  ...
  glutTimerFunc(1000/g_simulationsPerSecond, hairSimulationCallback, 0);
  glutPostRedisplay();
}
...
static void initSimulation() {
  ...
  for (int i = 0; i < g_bunnyMesh.getNumVertices(); i++) {
    ...
    Cvec3 norm = g_bunnyMesh.getVertex(i).getNormal();
    Cvec3 s = p + (norm * g_furHeight);
    s=bunnyRbt + s;
    g_tipPos.push_back(s);
  }
  hairSimulationCallback(0);
}
...
static void keyboard(const unsigned char key, const int x, const int y) {
  ...
  case 'h':
    ...
    << "b\t\tPlay/stop mesh animation\n"
    << "0\t\tIncrease number of subdivisions\n"
    << "9\t\tDecrease number of subdivisions\n"
    << "8\t\tDouble deformation speed\n"
    << "7\t\tHalve deformation speed\n"
  ...
  case 'f':
    ...
    break;
  case '0':
    g_subDivisions = min(g_subDivisions + 1, 7);
    cout << g_subDivisions << "subdivisions" << endl;
    g_mesh = g_meshOriginal;
      for (int i =0; i < g_subDivisions; i++) {
        subdivideMeshCatmullClark(g_mesh);
      }
    ...
    break;
  case '9':
    g_subDivisions = max(g_subDivisions - 1, 0);
    cout << g_subDivisions << "subdivisions" << endl;
    g_mesh = g_meshOriginal;
      for (int i =0; i < g_subDivisions; i++) {
        subdivideMeshCatmullClark(g_mesh);
    ...
    break;
  case '8':
    g_msBetweenBubbleFrames = max(1, g_msBetweenBubbleFrames/2);
    cout << "Bubbling speed doubled to " << g_msBetweenBubbleFrames << " ms between frames" << endl;
    break;
  case '7':
    g_msBetweenBubbleFrames = min(1000000, g_msBetweenBubbleFrames*2);
    cout << "Bubbling speed halved to " << g_msBetweenBubbleFrames << " ms between frames" << endl;
    break;
  }
  glutPostRedisplay();
}
...
static void initGlutState(int argc, char * argv[]) {
  ...
  glutCreateWindow("Assignment8 8");                       // title the window
...
static void initMaterials() {
  ...
  g_meshMat->getUniforms().put("uColor", Cvec3f(1, 0, 1));

  // bunny material
  g_bunnyMat.reset(new Material("./shaders/basic-gl3.vshader", "./shaders/bunny-gl3.fshader"));
  g_bunnyMat->getUniforms().put("uColorAmbient", Cvec3f(0.45f, 0.3f, 0.3f)).put("uColorDiffuse", Cvec3f(0.2f, 0.2f, 0.2f));
  ...
  bunnyShellMatPrototype.getRenderStates().blendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA).enable(GL_BLEND).disable(GL_CULL_FACE); // Set blending mode, enable blending, disable culling

  // allocate array of materials
  g_bunnyShellMats.resize(g_numShells);
  for (int i = 0; i < g_numShells; i++) {
    g_bunnyShellMats[i].reset(new Material(bunnyShellMatPrototype)); // copy from the prototype, but set a different exponent for blending transparency
