...
inline Matrix4 transFact(const Matrix4& m) {
  Matrix4 trans = m;
  trans(0, 0) = trans(1, 1) = trans(2,2) = 1; // 1 for each value along the diagonal.
  trans(0, 1) = trans(0, 2) = trans(1, 0) = trans(1, 2) = trans(2, 0)  = trans(2, 1) = 0; // Zero out everything else except the right-most column.
  return trans;
}

inline Matrix4 linFact(const Matrix4& m) {
  Matrix4 lin = m;
  lin(0, 3) = lin(1, 3) = lin(2, 3) = 0; // Zero out the translation factor.
  return lin;
}
