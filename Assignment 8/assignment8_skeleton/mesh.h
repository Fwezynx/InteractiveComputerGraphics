...
public:
  ...
  void updateNormals() {
    for (int i = 0; i < getNumVertices(); i++) {
      getVertex(i).setNormal(Cvec3());
    }
    for (int i = 0; i < getNumFaces(); i++) {
      Face face = getFace(i);
      for (int j = 0; j < face.getNumVertices(); j++) {
        face.getVertex(j).setNormal(face.getVertex(j).getNormal() + face.getNormal());
      }
    }
    for (int i = 0; i < getNumVertices(); i++) {
      Vertex vertex = getVertex(i);
      // Do nothing if vector is near zero to avoid aessert
      if (norm2(vertex.getNormal()) > CS175_EPS2) {
